...
def _parse(argv: list[str]):
    sys.argv = ["mflux-generate-z-image-controlnet", "--prompt", "p", *argv]
    return z_image_turbo_generate_controlnet.build_parser().parse_args()


@pytest.mark.fast
def test_union_controlnet_global_strength_defaults_to_one():
    ...
    args = _parse(["--control", "canny:a.png:0.85"])
...
@pytest.mark.fast
def test_union_controlnet_global_strength_can_still_be_set():
    args = _parse(["--control", "pose:a.png", "--controlnet-strength", "0.5"])
